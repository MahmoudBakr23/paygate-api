...
module Adapters
  ...
  class CheckoutAdapter < BaseAdapter
    ...
    def charge(amount:, currency:, token:, metadata: {})
      # Sandbox simulation: tok_mada is a well-known test token generated by the
      # Paygate SDK when Checkout.com Frames is not available. Simulate a captured
      # charge without hitting the Checkout.com API.
      if sandbox? && token == "tok_mada"
        return ChargeResult.new(
          provider_charge_id: "pay_mada_sandbox_#{SecureRandom.hex(8)}",
          status: "captured",
          failure_code: nil,
          failure_message: nil
        )
